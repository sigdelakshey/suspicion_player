...
public class GoodGuesser extends Bot
{
    Random r = new Random();
    ...
    private int countGuestNames(int p, ArrayList<String> assignedIDs, HashMap<String, HashMap<String, Integer>> guestNameCounts)
    {
        int totalCount=0;
        ...
        else for(String gid : players.get(otherPlayerNames[p]).possibleGuestNames)
        {
            ...
            {
                assignedIDs.add(gid);
                totalCount+=countGuestNames(p+1, assignedIDs, guestNameCounts);
    ...
    private Guess getBestGuess(int p, ArrayList<String> assignedIDs, GuestNameStats stats, int bestCount)
    {
        Guess bestGuess=null;
        //it's possible to return earlier from this part of the search
        //you could back out if count of possible points up to this point based on assignedIDs becomes less
        //than the sum of the remaining maximum possible points (based on maximum frequency count of assignable IDs
        // for each remaining player to assign)
        if(p>=otherPlayerNames.length)
        {
            int c=0;
            String guessStr="";
            for(int x=0;x<otherPlayerNames.length;x++)
            {
                c += stats.guestNameCounts.get(otherPlayerNames[x]).get(assignedIDs.get(x));
                guessStr+=otherPlayerNames[x] + ","+assignedIDs.get(x)+":";
            }
            guessStr=guessStr.substring(0,guessStr.length()-1);
            if(c>bestCount)
            {
                bestGuess=new Guess(guessStr, c, stats.totalCount);
            }
        }
        else for(String gid : players.get(otherPlayerNames[p]).possibleGuestNames)
        {
            if(!assignedIDs.contains(gid))
            {
                assignedIDs.add(gid);
                Guess temp;
                if((temp = getBestGuess(p+1, assignedIDs, stats, bestCount))!=null)
                {
                    if(temp.guessCount>bestCount)
                    {
                        bestCount=temp.guessCount;
                        bestGuess=temp;
                    }
                }
                assignedIDs.remove(gid);
    ...
    public class Guess
    {
        ...
        public int totalCount; // use this term to normalize guesscount*7 above to get expected score.
                        // i.e. utility = (guestCount*7)/totalCount;

        public Guess(String guess, int guessCount, int totalCount)
        {
            this.guess=guess;
    ...
    private Guess getBestGuess(GuestNameStats stats)
    {
        return getBestGuess(0, new ArrayList<String>(), stats, 0);
    }
    ...
    private GuestNameStats countGuestNames()
    {
        int totalCount=0;
        ...
        totalCount += countGuestNames(0,new ArrayList<String>(), guestNameCounts);
    ...
    private void sortPossibleGuestNames(HashMap<String, Integer> guestNameCounts)
    {
        String rval="";
        for(String k:players.keySet())
        {
    ...
    private void cleanUpZeroCountIDs(GuestNameStats stats)
    {
        for(String name:otherPlayerNames)
        {
            ...
            String zeroNames="";
            for(String guestID: counts.keySet())
            {
                if(counts.get(guestID)>0)
                {
                    nonzero.add(guestID);
                }
                else zeroNames=guestID + ",";
            }
            if(nonzero.size()<=0) throw new RuntimeException("No non-zero counts for player " + name + "for any of " + zeroNames);
    ...
    {
        GuestNameStats stats = countGuestNames();
    ...
    public String reportGuesses()
    {
        ...
        String rval = getBestGuess(stats).guess;
        //System.out.println("BEST GUESS: " + rval);

        return rval;
    }

    public GoodGuesser(String playerName, String guestName, int numStartingGems, String gemLocations, String[] playerNames, String[] guestNames)
    {
